 ...
 public Inventory() {
		...
		leaveButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(Combat.inCombat==true) Combat.menuPanel.setVisible(true);
				inventoryPanel.setVisible(false);
				if(Combat.inCombat == false) AdventureManager.mainPanel.setVisible(true);
		...
		useButton.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				if(items.get(currentItem).type.equals("Item")) {
	 ...
	 if(currentItem>=items.size()) {
		...
		usePanel.setVisible(false);
			...
			if(items.get(currentItem).type.equals("Weapon")) {
				infoArea.setText("This is a blank space! Pick up some items to equip/use!");
				usePanel.setVisible(true);
				useButton.setText("Equip");
				dropButton.setText("Drop");
			}
			if(items.get(currentItem).type.equals("Armor")) {
				infoArea.setText(items.get(currentItem).name + "\n\n" + items.get(currentItem).type + "\n\n" + items.get(currentItem).description + "\n\nIncreases max health by " + items.get(currentItem).value + " health!");
				usePanel.setVisible(true);
				useButton.setText("Equip");
				dropButton.setText("Drop");
			}
...
public void equipArmor(Item t) {
	 currentArmor = t;
	 Adventurer.maxHealth = 20 + currentArmor.value;
	 AdventureManager.healthInfo.setText("Health: " + Adventurer.health + "/" + Adventurer.maxHealth + "   Gold: " + AdventureManager.gold );
 }
